...
const CasinoApp = {
    ...
    showWinModal(title, subtitle, detailsHtml, totalWin) {
        ...
        document.getElementById('win-modal-details').innerHTML = detailsHtml || '<div class="text-center text-gray-400 text-xs py-2">?°Ê?Á¥?/div>';
    ...
    baccaratScratchCards: [], // ËøΩËπ§ Canvas ÂØ¶È?
    ...
    async fetchPoints() {
        try {
            ...
            this.points = data.totalMaryScore || 0; // ?πÁÇ∫‰ΩøÁî®Â∞èÁë™?âÁ?Á¢?(Casino Chips)
            
            // ÁÆ°Á??°ÁÑ°?êÈ??∏‰?Ë≠?            if (ADMIN_USER_IDS.includes(this.user.userId)) {
                this.points = 999999;
            }
    ...
    // ???πÁ?ÔºöË≥≠?¥ÂÖ®?¢Êîπ?®„ÄåÂ??®Á?Á¢ºÂà∂???åÂ??™Ë?Á©çÂ?)ÔºåÂ?Ê≠§‰Ωø??playSmallMary ?≤Ë??åÊ≠•
    ...
    openGame(gameType) {
        ...
        // Â∞èÁë™?âË??ÅË?Ê©üÈÉΩ‰∏çÈ?Ë¶ÅÂ??®Á?Á¢ºÂ?
    ...
    backToLobby() {
        ...
        // ?±Ë?Â∫ïÈÉ®Á±åÁ¢º??        document.getElementById('casino-footer').classList.add('translate-y-full');
    ...
    drawRouletteWheel() {
        ...
        for (let i = 0; i < 38; i++) {
            ...
            // ‰ΩøÁî®Â∞èÊï∏Èªû‰ª•?≤Âá∫?æÊé•Á∏?            gradientParts.push(`${color} ${(i * anglePerSlice).toFixed(2)}deg ${((i + 1) * anglePerSlice).toFixed(2)}deg`);
        }
        
        // Â∞áËµ∑ÂßãË?Â∫¶ÂæÆË™øÂ???sliceÔºåË??∏Â??ΩÂú®?áÂΩ¢Ê≠?∏≠Â§?        wheel.style.background = `conic-gradient(from ${-(anglePerSlice / 2)}deg, ${gradientParts.join(', ')})`;
        ...
        for (let i = 0; i < 38; i++) {
            ...
            // Ë®≠Â??∫Á?Â∞çÂ?‰ΩçÔ?‰∏¶Â?ËÆäÂΩ¢?üÈ?Ë®≠Âú®‰∏≠Â?ÔºåÊ?Â≠óÊîæ?®Ê?‰∏äÊñπ?äÁ∑£
    ...
    initRouletteBoard() {
        ...
        // ÁæéÂ?Ëº™Áõ§?çÁΩÆ (3Ê¨?12?óÔ?Â∑¶È???0 ??00)
        ...
        // ?∏Â? 1-36 ?áÊ©´??Columns) ?ÑÂÆπ??(Grid 13 col x 3 row)
        // Â§öÂá∫ 1 col Áµ?"2 to 1" (Column bets)
        ...
        [topRow, midRow, botRow].forEach((row, rowIndex) => {
            row.forEach((num, colIndex) => {
                ...
                // ÈªûÊ??ºÂ??ÇËß∏?º‰?ÂÆÆÊ†ºË¶ñÁ?
                cell.onclick = (e) => {
                    e.stopPropagation();
                    this.openRouletteBetPopup(num, cell, e);
                };

                // ?¢Á?‰∏ãÊ≥®Á±åÁ¢º?ÑÂ?‰ΩçÈå®Èª?(Anchor Points)
                // ?ô‰??®È?‰∏çÂÖ∑?ôÈ??äÂ??ΩÔ??ÖÁî®?ºË?Á±åÁ¢ºÊ∫ñÁ¢∫ÂÆö‰??®Ê†ºÁ∑ö‰∫§?åË?
                if (colIndex < 11) {
                    // Vertical Split Anchor (?≥È??å‰∏≠Èª?
                ...
                if (rowIndex < 2) {
                    // Horizontal Split Anchor (‰∏ãÈ??å‰∏≠Èª?
        ...
        // Â§ñÈÉ®?ºÊ≥®?Ä (Outside Bets) - ?æ‰??¢Ô??¥Êé•?†Âú® numbersGrid Ë£°‰ª•ÂÆåÁ?Â∞çÈ? 12 Ê¨?        const dozens = [
        ...
        dozens.forEach(d => {
            ...
            el.className = 'roulette-cell cell-trans col-span-4 mt-[1px] relative'; // Ë∑?4 Ê¨?            el.innerText = d.label;
            el.onclick = () => this.placeRouletteBet(d.id, el);
            numbersGrid.appendChild(el);
        });
        // Ë£ú‰??ÄÂæå‰??ºÁ©∫??(2 to 1 ?ÑÊ≠£‰∏ãÊñπ)
        ...
        evenOdds.forEach(e => {
            const el = document.createElement('div');
            el.className = 'roulette-cell cell-trans col-span-2 mt-[1px] text-[10px] sm:text-xs relative'; // Ë∑?2 Ê¨?            if (e.cls) {
    ...
    openRouletteBetPopup(num, cellElement, event) {
        if (this.isSpinning) {
            this.showTicker("?äÊà≤?≤Ë?‰∏?, "?ãÁ??üÈ??°Ê?‰∏ãÊ≥®?îÔ?");
            return;
        }

        // ÁßªÈô§?æÊ??ÑÂ?Ë∑≥Ë?Á™?        const existingPopup = document.getElementById('roulette-bet-popup');
        if (existingPopup) {
            existingPopup.remove();
        }

        // Â¶ÇÊ?ÈªûÊ??ÑÊòØ?üÊú¨Â∑≤È??üÁ??ºÂ?ÔºåÂ∞±?∂‰??ØÈ???        if (this.currentPopupCell === cellElement) {
        ...
        // ?≤Ê≠¢ÈªûÊ?ÂΩàÁ??ßÈÉ®?ÇÈ??âÂ?Á™?        popup.onclick = (e) => e.stopPropagation();
        ...
        const createBtn = (label, betId, colorClass = 'bg-gray-800 text-gray-400') => {
            ...
            if (isValid) {
                btn.onclick = () => {
                    // Â¶ÇÊ???anchor (ËßíÊ≥®?ñÂ?Ê≥?ÔºåÂ∞±??anchor ‰ΩúÁÇ∫ÂÆö‰?ÈªûÔ??¶Â???cellElement (?ÆÊ≥®)
        ...
        // Ê∏≤Ê?ÂæåÂ?Ë™øÊï¥‰ΩçÁΩÆ?øÂ?Ë∂ÖÂá∫?äÁ?
        requestAnimationFrame(() => {
            ...
            if (finalTop < 0) {
                // Â¶ÇÊ?‰∏äÊñπÁ©∫È?‰∏çË∂≥ÔºåÊîæ?∞‰???                finalTop = cellRect.bottom - containerRect.top + 10;
            }
    ...
    placeRouletteBet(betId, cellElement) {
        ...
        if (this.points < this.currentChip) {
            this.showAlert("Á©çÂ?‰∏çË∂≥", "?®Á?È§òÈ?‰∏çË∂≥‰ª•ÈÄ≤Ë?‰∏ãÊ≥®Ôº?);
            return;
        }
    ...
    initSicboBoard() {
        ...
        // Row 1: Â∞?/ ?®Â? / Â§?        const row1 = createRow('grid-cols-[1fr_auto_1fr]');
        createCell('small', 'Â∞?SMALL', '4-10 (1Ë≥?)', row1);
        createCell('any_triple', '?®Â? ANY TRIPLE', '1Ë≥?4', row1);
        createCell('big', 'Â§?BIG', '11-17 (1Ë≥?)', row1);
        board.appendChild(row1);

        // Row 2: ?πÂ??çÈ™∞ (Specific Triples)
        const row2 = createRow('grid-cols-6');
        [1,2,3,4,5,6].forEach(n => createCell(`triple_${n}`, `${n}${n}${n}`, '1Ë≥?50', row2));
        board.appendChild(row2);

        // Row 3: Â∞çÂ? (Specific Doubles)
        const row3 = createRow('grid-cols-6');
        [1,2,3,4,5,6].forEach(n => createCell(`double_${n}`, `${n}${n}`, '1Ë≥?', row3));
        board.appendChild(row3);

        // Row 4: Á∏ΩÂ? (Total Sums)
        const row4 = createRow('grid-cols-7');
        const sumsRow1 = [4, 5, 6, 7, 8, 9, 10];
        const sumsRow2 = [11, 12, 13, 14, 15, 16, 17];
        const getSumOdds = (sum) => {
            if (sum === 4 || sum === 17) return '1Ë≥?0';
            if (sum === 5 || sum === 16) return '1Ë≥?8';
            if (sum === 6 || sum === 15) return '1Ë≥?4';
            if (sum === 7 || sum === 14) return '1Ë≥?2';
            if (sum === 8 || sum === 13) return '1Ë≥?';
            return '1Ë≥?';
        };
        ...
        // Row 5: ?ÆÈ™∞ (Single Dice)
        const row6 = createRow('grid-cols-6');
        [1,2,3,4,5,6].forEach(n => createCell(`single_${n}`, `È™?${n}`, '1Ë≥?', row6));
    ...
    spinSicbo() {
        ...
        if (Object.keys(this.sicboBets).length === 0) {
            this.showTicker("Ë´ãÂ?‰∏ãÊ≥®", "?®È?Ê≤íÊ??æÁΩÆ‰ªª‰?Á±åÁ¢º?îÔ?");
            return;
        }
        ...
        // 3 ÁßíÂ??úÊ≠¢‰∏¶Á?ÁÆ?        setTimeout(() => {
            clearInterval(animationInterval);
            
            // Ê±∫Â??ÄÁµÇÈ???            const results = [
            ...
            for (const [betId, amount] of Object.entries(this.sicboBets)) {
                ...
                if (betId === 'small' || betId === 'big') winRatio = 2; // 1Ë≥? -> ?¨È?x2
                else if (betId === 'any_triple') winRatio = 25; // 1Ë≥?4 -> ?¨È?x25
                else if (betId.startsWith('triple_')) winRatio = 151; // 1Ë≥?50 -> ?¨È?x151
                else if (betId.startsWith('double_')) winRatio = 9; // 1Ë≥? -> ?¨È?x9
                else if (betId.startsWith('single_')) {
                    const singleNum = parseInt(betId.split('_')[1]);
                    const occurrences = counts[singleNum] || 0;
                    winRatio = occurrences + 1; // 1È°ÜË?1, 2È°ÜË?2, 3È°ÜË?3 (+1?¨È?)
                }
            ...
            setTimeout(() => {
                if (totalWin > 0) {
                    this.showTicker(`‚≠?È™∞Â? ${results.join(', ')} (Á∏ΩÂ?${sum})ÔºÅË?Âæ?${totalWin.toLocaleString()} Á©çÂ? ‚≠ê`, 'win');
                    this.points += totalWin;
                } else {
                    this.showTicker(`È™∞Â? ${results.join(', ')} (Á∏ΩÂ?${sum})ÔºÅÂ??ØÊ??™‰∏≠?é„ÄÇ`, 'lose');
                }
    ...
    spinRoulette() {
        ...
        if (Object.keys(this.rouletteBets).length === 0) {
            this.showAlert("Ë´ãÂ?‰∏ãÊ≥®", "?®È?Ê≤íÊ??æÁΩÆ‰ªª‰?Á±åÁ¢º?îÔ?");
            return;
        }

        // ÁßªÈô§?àÂ??Ñ‰∏≠?éÁâπ??        document.querySelectorAll('.winning-cell').forEach(el => {
            el.classList.remove('winning-cell', 'winning-bet');
        });
        ...
        // Á¥ØÁ??ãË?ÔºåÁ¢∫‰øùÊ∞∏?†È??ÇÈ?ËΩ?(Â§öË? 5 ??
        ...
        setTimeout(() => {
            ...
            for (const [betId, amount] of Object.entries(this.rouletteBets)) {
                let winRatio = 0; // ?ÖÂê´?¨È??ÑË???                
                if (betId === `num_${winningStr}` || betId === winningStr) {
                    winRatio = 36; // ?Æ‰??üÁ¢º 1Ë≥?5
                } 
            ...
            // ËÆì‰∏≠?é‰?ÁΩÆÁôº??            const winningIds = [winningStr];
            ...
            // Âª∂ÈÅ≤ 500ms ËÆìÁé©ÂÆ∂ËÉΩ?ãÂà∞?ºÂ??πÊ?ÔºåÂ?Ë∑≥Âá∫ alert
            setTimeout(() => {
                // ?¥Êñ∞ÁµêÊ?
                if (totalWin > 0) {
                    this.showTicker(`‚≠?Ëº™Áõ§?ãÂá∫ ${winningStr}ÔºÅË?Âæ?${totalWin.toLocaleString()} Á©çÂ? ‚≠ê`, 'win');
                    this.points += totalWin;
                } else {
                    this.showTicker(`Ëº™Áõ§?ãÂá∫ ${winningStr}ÔºÅÂ??ØÊ??™‰∏≠?é„ÄÇ`, 'lose');
                }
    ...
    placeBaccaratBet(type) {
        ...
        if (this.points < this.currentChip) {
            this.showAlert("Á©çÂ?‰∏çË∂≥", "?®Ê??âË∂≥Â§†Á?Á©çÂ??Ø‰?Ê≥?);
            return;
        }
    ...
    async handleBaccaratDeal() {
        ...
        if (totalBet === 0) {
            this.showAlert("?êÁ§∫", "Ë´ãÂ?‰∏ãÊ≥®Ôº?);
            return;
        }

        this.isSpinning = true;
        document.querySelectorAll('.baccarat-cell').forEach(el => {
            el.classList.remove('winning-cell', 'winning-bet');
        });
    ...
    baccaratSettle() {
        ...
        setTimeout(() => {
            if (totalWin > 0) {
                this.points += totalWin;
                document.querySelectorAll('.player-wallet-text').forEach(el => el.innerText = Math.floor(this.points).toLocaleString());
                this.showTicker(`?≠Â?Ë¥èÂ? ${Math.floor(totalWin)} Á©çÂ?ÔºÅ`, "win");
                this.showAlert(winMsg, `?≠Â?Ë¥èÂ? ${Math.floor(totalWin)} Á©çÂ?ÔºÅ`, true);
            } else {
                this.showTicker("?™‰∏≠?éÔ??çÊé•?çÂé≤Ôº?, "lose");
            }
